...
attacco_2 (){
	echo -e "${green}Benvenuto nel LIVELLO 2 esploratore della rete.\nUn attaccante è entrato nella tua rete e \
sta cercando di accedere alla tua macchina. Sniffa la rete per capire quali attacchi sono in corso, poi configura \
Snort per rilevarli tutti.${default}

Comandi utili:${orange}
- KILLA IL LIVELLO: ${cyan}killall bash sleep${orange}
- AVVIA LIVELLO: ${cyan}/root/Snort_Hackathon/hackathon-tools/attack/level2${orange}
- CHECK DEL LIVELLO: ${cyan}/root/Snort_Hackathon/hackathon-tools/evaluation/evaluate_level2${default}
...
Buona difesa!"
	frasi=(
		"Le password sono solo sfide"
		"Il mio talento è invisibile"
		"Navigo nell'oscurità"
	...
	(
	 while true; do
		# ping
	 	sshpass -p 'test' ssh test@"$my_ip" > /dev/null 2>&1
		sleep 1
	 done
	) &
...
# Level 1 check
PASSWORD="Sn0rt!Y0urGuaRdian"
echo "Level2 Password:"
read -s INPUT_PASSWORD

# Controlla se la password è errata
if [ "$INPUT_PASSWORD" != "$PASSWORD" ]; then
    echo "Password per il livello 2 errata. Esco.."
    exit 1
fi
