...
def check_snort_rules(rules_file, log_file):
    ...
    with open(rules_file, 'r') as f:
        for line in f:
            rule_id=0
            msg=0
            if '80' in line and 'flags:P' in line and 'content' in line and 'script' in line and line.startswith("alert tcp"):
                #print("flag FPU: "+ line)
                rule_id = extract_rule_id(line)
                msg = extract_msg(line)
                if rule_id and msg and sid_xss==0:
                    sid_xss=rule_id
                    rules_set.add(rule_id+"|"+msg)
            if '80' in line and 'flags:P' in line and 'content' in line and 'login' in line and line.startswith("alert tcp"):
                #print("flag FPU: "+ line)
                rule_id = extract_rule_id(line)
                msg = extract_msg(line)
                if rule_id and msg and sid_sql==0:
                    sid_sql=rule_id
                    rules_set.add(rule_id+"|"+msg)

	# Controlla che le regole attive siano loggate negli alert
    if sid_sql !=0 and sid_xss!=0:
        ...
        if not rules_set:
            print("Livello 5 SUPERATO!")
            print("Flag del completamento dell'hackathon: L1nuXD@y!2024_Rom3")
            subprocess.run(['killall','bash','sleep'], capture_output=True, text=True)
            print("Processi del level5 killati. COMPLIMENTI HAI COMPLETATO L'HACKATHON!")
            os._exit(0)
    print ("Livello 5 non ancora superato. Aspetta sempre 10-15sec che i log si aggiornino.")
    os._exit(0)
