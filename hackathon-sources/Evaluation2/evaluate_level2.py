...
def check_snort_rules(rules_file, log_file):
	...
	with open(rules_file, 'r') as f:
		for line in f:
			rule_id=0
			msg=0
			if '22' in line and 'flags:P' in line and "threshold" in line and 'count' in line and line.startswith("alert tcp"):
				#print("flag FPU: "+ line)
				rule_id = extract_rule_id(line)
				msg = extract_msg(line)
				if rule_id and msg and sid_sshbrute==0:
					sid_sshbrute=rule_id
	...
	if sid_sshbrute !=0:
		...
		if not rules_set:
			...
			print("PASSWORD per il Livello 3: Sn0rtIsN0t@Fir3walL")
			subprocess.run(['killall','bash','sleep'], capture_output=True, text=True)
			print("Processi del level2 killati. Puoi procedere con il livello successivo.")
			os._exit(0)
	print ("Livello 2 non ancora superato. Aspetta sempre 10-15sec che i log si aggiornino.")
	os._exit(0)
